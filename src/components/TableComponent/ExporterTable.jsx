...
export const exportPdfWithCustomTitles = async (
  columns,
  data,
  fileName,
  pdfTitle,
  ...
  try {
    ...
    toast.error(
      "Failed to export PDF: One or more images could not be loaded.',",
    );
  }
};

export const exportPdfWithCustomTitle = async (
  columns,
  data,
  fileName,
  pdfTitle,
...
export const ExporterTable = ({
  ...
  var data = table.options.data.map((row) => {
    ...
    exportColumns.forEach((col) => {
      ...
      const columnDef = table
        .getAllColumns()
        .find((c) => c.id === col.field.replace("_", "."))?.columnDef;
  ...
  const nestedAccessorKeyColumns = (table?.getVisibleFlatColumns() ?? [])
    .filter((col) => get(col, "columnDef.accessorKey", "")?.includes("."))
    .map((col) => get(col, "columnDef.accessorKey", ""));

  if (nestedAccessorKeyColumns?.length) {
    data = data.map((item) => ({
      ...item,
      ...nestedAccessorKeyColumns.reduce(
        (pre, cur) => ({
          ...pre,
