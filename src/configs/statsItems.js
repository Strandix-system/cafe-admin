export const getSuperAdminStats = (statsData) => [
  { label: "Total Cafes", value: statsData?.totalCafe ?? 0, path: "/cafes" },
  {
    ...
    path: "/cafes?tab=active",
  },
  {
    label: "Inactive Cafes",
    value: statsData?.totalInActive ?? 0,
    path: "/cafes?tab=inactive",
  },
  {
    label: "Total Income",
...
export const getAdminStats = (statsData) => [
  {
    ...
    value: statsData?.totalCustomer ?? 0,
    path: "/customer",
  },
  {
    label: "Total Orders",
    value: statsData?.totalOrder ?? 0,
    path: "/order-management?tab=2",
  },
  {
    label: "Total Income",
    value: `₹${statsData?.totalIncome?.toLocaleString() ?? 0}`,
